...
%extract the laser frame's origin in the world frame (P1) from the transformation matrix Tl
P1 = Tl*[0 0 1]';
x1=P1(1);     y1=P1(2); %this point is the laser beam's origin in the world frame
[ I1, J1 ] = XYtoIJ(x1, y1, Xmax, Ymax, R, C); % find the corresponding pixel
for a = angleSpan/2 : - angleStep: - angleSpan/2
    %now calculate the laser beam's farthest possible end point in the scanner frame
    Xl = rangeMax * cos(a);
    Yl = rangeMax * sin(a);
    
    %Transform this point in world frame
    P2 = Tl*[Xl Yl 1]';
    x2=P2(1); y2=P2(2);
    
    %clip the laser beam (line segment from P1 to P2) to the world's boundary polygon
    edge = clipLine([x1,y1,x2-x1,y2-y1],[0 Xmax 0 Ymax]);
    %assume the laser origin is always inside the map
    x2 = edge(3); y2 = edge(4);
    
    % now compute if the laser beam intersects an obstacle in the map
    ...
    if (isinf(Io) || isinf(Jo))
        ...
        %compute target point (obtsacle poitn that reflected back the light) in world coordinates
        [ xTarget, yTarget ] = IJtoXY(Io, Jo, Xmax, Ymax, R, C);
        % compute distance from laser beam origin to target point
